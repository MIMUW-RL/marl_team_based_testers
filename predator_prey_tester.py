...
class HnsSideChannel(SideChannel):
    ...
    def on_message_received(self, msg):
        ...
        if s == "Environment/HidersCaptured":
            f = msg.read_float32()
            self.hiders_captured = f
            print("Hiders captured %:", f)
...
parser.add_argument(
    "-t0", type=str, help="text file with checkpoint paths to load (team prey)"
)
parser.add_argument(
    "-t1", type=str, help="text file with checkpoint paths to load (team predators)"
)
...
parser.add_argument("--game_params", help="game controller parameters json file")
parser.add_argument("--arena_params", help="arena parameters json file")
...
additional_args = []
if args.game_params:
    additional_args.append("game_params=" + args.game_params)
if args.arena_params:
    additional_args.append("arena_params=" + args.arena_params)
...
for episode in range(EPISODES):
    ...
    while not done:
        ...
        for i, team_id in enumerate(behavior_names):
            ...
            for agent_id in decision_steps[team_id]:
                ...
                team_rewards[team_id] += decision_steps[team_id].group_reward[
                    0
                ] / len(decision_steps[team_id].group_reward)
    ...
    endTime = time.time()
    print("HideAndSeekAgent?team=0")
    print(agent_rewards["HideAndSeekAgent?team=0"].values())
    print("HideAndSeekAgent?team=1")
    print(agent_rewards["HideAndSeekAgent?team=1"].values())
    reward_solo_hiders = (
    ...
    if team_won == 0:
        reward_team_hiders = 1
        reward_team_seekers = -1
    else:
        reward_team_hiders = -1
        reward_team_seekers = 1
    ...
    hiders_captured = int(round(sc.hiders_captured * agents))
    ...
    total_winratio += sc.winratio
    total_hiders_captured += hiders_captured

print("Avg time hidden:", total_time_hidden / EPISODES)
print("Avg hider winratio:", total_winratio / EPISODES)
print("Avg hiders captured:", total_hiders_captured / EPISODES)
...
print("Avg hiders team reward:", total_reward_team_hiders / EPISODES)
print("Avg seekers team reward:", total_reward_team_seekers / EPISODES)
env.close()
endTotal = time.time()
